...
$(window).resize(function() {
    ...
    if (width < 440) {
        $('#nav-items').hide();
        $('#sticky-form-inline').hide();
        $('#certificate-preview').hide();
        $('#self-img').hide();
    } else {
        $('#nav-items').show();
        $('#sticky-form-inline').show();
...
$('#feesPayGooglePay,#feesPhonePay,#feesPayTmPay,#feesUPIPay').on('change', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status) {
                $('#preview').hide();
                window.location.href = "/fees-pay-confirmation?status=Success&o=" + response.oo;
            } else {
                window.location.href = "/fees-pay-confirmation?status=failed&o=" + response.oo;
...
$(document).on('click', '.downloadPopup', function() {
    ...
    var button = THIS.data('button');
    $("#downloadPopup").modal();
    $("#downloadPopup .modal-heading").html(title);
    $('.from_title').val(title);
    $("#downloadPopup .iq_from").val(title);
...
$(document).on('click', '.downloadCurriculum', function() {
    ...
    var title = THIS.data('title');
    var button = THIS.data('button');
    $("#downloadCurriculum").modal();
    $("#downloadCurriculum .modal-heading").html(title);
    $('.from_title').val(title);
    $("#downloadCurriculum .iq_from").val(title);
...
$(document).on('click', '.commanModalCourse', function() {
    ...
    var title = THIS.data('title');
    var button = THIS.data('button');
    $("#commanModalCourse").modal();
    $("#commanModalCourse .modal-heading").html(title);
    $('.from_title').val(title);
    $("#commanModalCourse .iq_from").val(title);
...
$(document).on('click', '.downloadsyllbus', function() {
    ...
    var title = THIS.data('title');
    var button = THIS.data('button');
    $("#downloadsyllbus").modal();
    $("#downloadsyllbus .modal-heading").html(title);
    $('.from_title').val(title);
    $("#downloadsyllbus .iq_from").val(title);
...
$(document).on('click', '.inquerySide', function() {
    ...
    $("#sidemodalclass").modal();
    $("#sidemodalclass #myModalLabel2").html(title);
    $('.from_title').val(title);
    $("#sidemodalclass .iq_from").val(title);
...
$(document).on('click', '.videotrigger', function() {
    ...
    var button = THIS.data('button');
    $("#videotrigger").modal();
    $("#videotrigger .modal-heading").html(title);
    $('.from_title').val(title);
    $("#videotrigger .iq_from").val(title);
...
$(document).on('click', '.videoreviews', function() {
    ...
    var button = THIS.data('button');
    $("#videoreviews").modal();
    $("#videoreviews .modal-heading").html(title);
    $('.from_title').val(title);
    $("#videoreviews .iq_from").val(title);
    $("#videoreviews #modal-placement-button").html(button);
});
...
var homeController = (function() {
    return {
        ...
        saveLeadBulkEmail: function(THIS) {
            var $this = $(THIS),
                data = $this.serialize();

            $.ajax({
                url: "/saveLeadBulkEmail",
                type: "POST",
                data: data,
                success: function(response) {
        ...
        get_admissionotp: function(THIS) {
            //alert("test otp");
            var $this = $(THIS);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == true) {
                        ...
                        $this.closest('.modal').modal('hide');
                        $("#enquirypopups").modal();
                        $("#enquirypopups .pastmobile").text($this.find('[name="mobile"]').val());
                        $("#enquirypopups .pastemail").text(email);
                        $('#enquirypopups').modal({
                            backdrop: "static",
                            keyboard: false
                        });
                        ...
                        $("#messagemodel").modal();
                        $('.imgclass').html('<img src="../public/img/message_alert.png" style="width: 50%;text-align: center;margin: auto;display: block;">');
                        $('#messagemodel h3').html("");
                        $('.failedhtml').html("<p class='text-center'>" + response.msg + ".</p>");
                        $('#messagemodel').modal({
                            backdrop: "static",
                            keyboard: false
                        });
        ...
        get_admissionotpBusiness: function(THIS) {
            //alert("test otp");
            var $this = $(THIS);
            var mobile = $this.find('[name="mobile"]').val();
            var email = $this.find('[name="email"]').val();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == true) {
                        ...
                        $("#enquirypopups").modal();
                        $("#enquirypopups .pastmobile").text(mobile);
                        $("#enquirypopups .pastemail").text(email);
                        $('#enquirypopups').modal({
                            backdrop: "static",
                            keyboard: false
                        ...
                        $('.imgclass').html('<img src="../public/img/message_alert.png" style="width: 50%;text-align: center;margin: auto;display: block;">');
                        $('#messagemodel h3').html("");
                        $('.failedhtml').html("<p class='text-center' style='color:red;'>" + response.msg + ".</p>");
                        $('#messagemodel').modal({
                            backdrop: "static",
                            keyboard: false
                        });
        ...
        saveAdmission: function(THIS) {
            //alert("test admission");
            var $this = $(THIS);
            var form = new FormData(THIS);
            $.ajax({
                url: "/saveAdmission",
            ...
            $.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                    ...
                    if (data.statusCode == 1) {
                        ...
                        if (payload.length > 0) {
                            $('.help-block').html('');
                            $('#certificateDownloadModal').modal();
                            $('#certificateDownloadModal .nit-form').html(payload);
                            $('#certificateDownloadModal .choke').off('click');
                        ...
                        $(".alert").remove();
                        $('.track-form .help-block').html('<strong>' + data.data.message + '</strong>');
                        $("#btn-login").html('<span><span>Continue</span></span>');
                    }
                }
            });
        ...
        GetCertificateIdOTP: function(THIS) {
        ...
        getCertificateno: function(THIS) {
            ...
            $.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                    if (data.statusCode && data.statusCode == 1) {
                        $('#certificateDownloadModal').modal('hide');
                        //$('.student-details').replaceWith(data.data.payload);
                        $('.certraid').hide();
                ...
                success: function(data, textStatus, jqXHR) {
                    ...
                    if (data.statusCode == 1) {
                        ...
                        if (payload.length > 0) {
                            ...
                            $('#leadsPayloadModal .nit-form').html(message);
                            ...
                            $('#leadsPayloadModal .choke').on('click', function() {
                                ...
                                if (target_id) {
                                    ...
                                    alert('You have not selected the technology / Your fees is due / Certificate has already been allplied');
                                    return false;
                                }
...
function offerclose() {
    ...
    $('.afterclose_open').show();
}

function openoffer() {
    $('.mob-offer').show();
    $('.offer-container').show();
    $('.afterclose_open').hide();
}
...
$('input[name="demo_date"]').datepicker({
    dateFormat: 'yy-mm-dd'
});
$('input[name="time_of_call"]').datepicker({
...
$(document).ready(function() {
    $(".webinar_resgister1").click(function() {
        $(".webinar_desksection1").toggle();
        $(".webinar_desksection2, .webinar_desksection3").css("display", "none");
    });
});
$(document).ready(function() {
    $(".webinar_resgister2").click(function() {
        $(".webinar_desksection2").toggle();
        $(".webinar_desksection1, .webinar_desksection3").css("display", "none");
    });
});

$(document).ready(function() {
    $(".webinar_resgister3").click(function() {
        $(".webinar_desksection3").toggle();
        $(".webinar_desksection1, .webinar_desksection2").css("display", "none");
    });
});